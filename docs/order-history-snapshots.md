...
The destination validates source identity, original timestamps/state/status, currencies, amounts, item parent graphs, related-record ownership and aggregate invoice/shipment/refund quantities and totals. It remaps order, item, product, customer, address and invoice relationships to destination IDs. Missing products/customers, unknown state/status mappings or conflicting document numbers fail rather than fabricate records.
